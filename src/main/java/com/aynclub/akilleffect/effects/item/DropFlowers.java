...
public class DropFlowers extends MainEffectKill {
    ...
    public void update(User user) {
        for (int i = 0; i < 30; i++) {
            ItemStack redDye = new ItemStack(Material.RED_ROSE, 1, (short) 1);
            Item ITEM = user.getPlayer().getWorld().dropItem(user.getPlayer().getLocation(), ItemFactory.create(redDye.getType(), (byte) 0, UUID.randomUUID().toString()));
