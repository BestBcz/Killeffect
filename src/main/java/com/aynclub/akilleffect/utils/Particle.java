...
public class Particle {
    ...
    public static void play(Location location, Effect effect, int data, Vector vector, float speed) {
        play(location, effect, data, data, (float) vector.getX(), (float) vector.getY(), (float) vector.getZ(), speed, 1);
    }
    ...
    public static void drawTornado(Location location, float radius, float increase) {
        float y = (float) location.getY();
        for (double t = 0; t < 50; t += 0.05) {
            float x = radius * (float) Math.sin(t);
            float z = radius * (float) Math.cos(t);
            play(location.clone().add(x, y, z), Effect.FLAME);
            y += 0.01f;
            radius += increase;
        }
    }
